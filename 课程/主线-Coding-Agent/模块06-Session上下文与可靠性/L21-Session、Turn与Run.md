# L21 Session、Turn与Run

> 建议时长：60–90 分钟｜讲解 40%｜实践 60%

## 1. 本节要解决的真实问题

会话历史和一次 Agent 运行为什么不能混为一谈？
...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。会话历史和一次 Agent 运行为什么不能混为一谈？ 这正是本节要补齐的环节。
...
Session 跨多轮保存消息，Turn 表示一次用户交互，Run 表示一次 Runtime 生命周期并拥有 run_id。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**实现最小 SessionStore**。
...
基础实验：完成“实现最小 SessionStore”，并保存一段真实运行输出。

进阶挑战：更换一个输入或失败条件，预测轨迹后再运行验证。

## 11. 自测问题

1. 会话历史和一次 Agent 运行为什么不能混为一谈？
2. 本节概念在执行轨迹的哪一步出现？
...
用三句话复述：本节问题、核心概念、代码变化。下一课：L22 上下文预算与压缩
