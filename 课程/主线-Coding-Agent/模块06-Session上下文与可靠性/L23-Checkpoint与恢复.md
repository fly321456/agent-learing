...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。长任务中断后怎样避免从头重跑？ 这正是本节要补齐的环节。
...
Checkpoint 只在稳定边界保存恢复状态；有副作用的工具需要幂等键或完成记录。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**保存并恢复 next_step 与事件**。
...
基础实验：完成“保存并恢复 next_step 与事件”，并保存一段真实运行输出。
...
用三句话复述：本节问题、核心概念、代码变化。下一课：L24 Config、错误、Retry与Timeout
