# L10 Tool Registry与通用路由

> 建议时长：60–90 分钟｜讲解 40%｜实践 60%

## 1. 本节要解决的真实问题

每增加一个 Tool 都写 if/elif 会发生什么？
...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。每增加一个 Tool 都写 if/elif 会发生什么？ 这正是本节要补齐的环节。
...
Tool Registry 用名称映射 Schema 和 handler，让 Runner 不依赖具体业务工具。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**用字典注册 calculator 与 current_time**。
...
基础实验：完成“用字典注册 calculator 与 current_time”，并保存一段真实运行输出。

进阶挑战：更换一个输入或失败条件，预测轨迹后再运行验证。

## 11. 自测问题

1. 每增加一个 Tool 都写 if/elif 会发生什么？
2. 本节概念在执行轨迹的哪一步出现？
...
用三句话复述：本节问题、核心概念、代码变化。下一课：L11 多Tool与同轮多调用
