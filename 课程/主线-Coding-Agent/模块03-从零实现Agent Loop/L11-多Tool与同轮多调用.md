...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。一轮响应包含多个 ToolCall 时如何处理？ 这正是本节要补齐的环节。
...
Runtime 必须按稳定顺序处理完整调用列表并逐个回传结果，不能只取第一个。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**让一轮同时调用 calculator 和 current_time**。
...
基础实验：完成“让一轮同时调用 calculator 和 current_time”，并保存一段真实运行输出。

进阶挑战：更换一个输入或失败条件，预测轨迹后再运行验证。

## 11. 自测问题

1. 一轮响应包含多个 ToolCall 时如何处理？
2. 本节概念在执行轨迹的哪一步出现？
...
用三句话复述：本节问题、核心概念、代码变化。下一课：L12 终止条件与错误
