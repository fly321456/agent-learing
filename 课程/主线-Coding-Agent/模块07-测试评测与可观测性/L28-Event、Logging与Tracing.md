# L28 Event、Logging与Tracing

> 建议时长：60–90 分钟｜讲解 40%｜实践 60%

## 1. 本节要解决的真实问题

失败后怎样知道 Agent 在哪一步出了问题？
...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。失败后怎样知道 Agent 在哪一步出了问题？ 这正是本节要补齐的环节。
...
Event 服务程序消费，Logging 服务人类阅读，Trace 用 run_id 串联完整运行。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**输出可重放 JSONL Trace**。
...
基础实验：完成“输出可重放 JSONL Trace”，并保存一段真实运行输出。

进阶挑战：更换一个输入或失败条件，预测轨迹后再运行验证。

## 11. 自测问题

1. 失败后怎样知道 Agent 在哪一步出了问题？
2. 本节概念在执行轨迹的哪一步出现？
...
- [课程首页](../../README.md)
- 模块检查点：agent-from-scratch/course-checkpoints/07-testing-evaluation/
