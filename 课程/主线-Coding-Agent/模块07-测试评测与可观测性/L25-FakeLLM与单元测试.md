...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。不调用真实模型怎样测试 Agent 决策链？ 这正是本节要补齐的环节。
...
FakeLLM 返回预设响应并记录请求，使纯文本、工具调用和异常路径确定可重复。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**为 Loop 编写第一个离线测试**。
...
基础实验：完成“为 Loop 编写第一个离线测试”，并保存一段真实运行输出。

进阶挑战：更换一个输入或失败条件，预测轨迹后再运行验证。

## 11. 自测问题

1. 不调用真实模型怎样测试 Agent 决策链？
2. 本节概念在执行轨迹的哪一步出现？
...
用三句话复述：本节问题、核心概念、代码变化。下一课：L26 契约、集成与E2E
