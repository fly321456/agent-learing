# L14 LLM接口与LLMResponse

> 建议时长：60–90 分钟｜讲解 40%｜实践 60%

## 1. 本节要解决的真实问题

Runner 为什么不能直接读取供应商对象？
...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。Runner 为什么不能直接读取供应商对象？ 这正是本节要补齐的环节。
...
BaseLLM 隔离供应商调用，LLMResponse 只表示一次模型调用的标准结果。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**抽出 BaseLLM、FakeLLM 与 LLMResponse**。
...
基础实验：完成“抽出 BaseLLM、FakeLLM 与 LLMResponse”，并保存一段真实运行输出。

进阶挑战：更换一个输入或失败条件，预测轨迹后再运行验证。

## 11. 自测问题

1. Runner 为什么不能直接读取供应商对象？
2. 本节概念在执行轨迹的哪一步出现？
...
用三句话复述：本节问题、核心概念、代码变化。下一课：L15 ToolManager与ToolResult

## 学习导航

- [课程首页](../../README.md)
- 模块检查点：agent-from-scratch/course-checkpoints/04-runtime-refactor/
