...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。工具成功、拒绝和失败如何统一表达？ 这正是本节要补齐的环节。
...
ToolSpec 描述工具，ToolManager 路由执行，ToolResult 用稳定状态表达结果。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**抽出 ToolSpec、ToolManager 与 ToolResult**。
...
基础实验：完成“抽出 ToolSpec、ToolManager 与 ToolResult”，并保存一段真实运行输出。
...
用三句话复述：本节问题、核心概念、代码变化。下一课：L16 Runner、RunResult与Event
