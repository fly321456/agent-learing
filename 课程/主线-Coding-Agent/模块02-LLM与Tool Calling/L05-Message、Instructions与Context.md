# L05 Message、Instructions与Context
...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。模型到底看到了哪些信息？ 这正是本节要补齐的环节。
...
Instructions 定义长期行为边界，Message 表达当前对话，Context 是本次调用实际携带的全部信息。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**打印并检查一次模型请求的输入列表**。
...
基础实验：完成“打印并检查一次模型请求的输入列表”，并保存一段真实运行输出。
...
用三句话复述：本节问题、核心概念、代码变化。下一课：L06 第一次Responses API文本调用
