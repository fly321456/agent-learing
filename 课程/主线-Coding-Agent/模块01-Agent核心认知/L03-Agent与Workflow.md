...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。什么时候应该用 Agent，什么时候固定流程更好？ 这正是本节要补齐的环节。
...
Workflow 的路径由人预先定义；Agent 的下一步由模型根据观察动态决定。稳定、可枚举的流程优先使用 Workflow。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**比较固定分支与动态决策两条执行轨迹**。
...
基础实验：完成“比较固定分支与动态决策两条执行轨迹”，并保存一段真实运行输出。

进阶挑战：更换一个输入或失败条件，预测轨迹后再运行验证。

## 11. 自测问题

1. 什么时候应该用 Agent，什么时候固定流程更好？
2. 本节概念在执行轨迹的哪一步出现？
...
用三句话复述：本节问题、核心概念、代码变化。下一课：L04 手工走一遍Think-Act-Observe

## 学习导航

- [课程首页](../../README.md)
- 模块检查点：agent-from-scratch/course-checkpoints/01-agent-concepts/
