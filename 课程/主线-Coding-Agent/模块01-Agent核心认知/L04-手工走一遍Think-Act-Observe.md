...
把 Coding Agent 想成一名受约束的开发者：它需要知道目标、选择动作、使用工具获得事实，再根据事实继续工作。模型调用工具后为什么还必须继续思考？ 这正是本节要补齐的环节。
...
Observe 是工具结果形成的新事实；只有把它送回决策者，系统才能根据结果调整下一步。

本节先把这一件事做正确，不提前加入后续模块的抽象。

## 6. 本节代码增量

修改目标：**完成第一个 ScriptedLLM 离线循环**。
...
基础实验：完成“完成第一个 ScriptedLLM 离线循环”，并保存一段真实运行输出。
